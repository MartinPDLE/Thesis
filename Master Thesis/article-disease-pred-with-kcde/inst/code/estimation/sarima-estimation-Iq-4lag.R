...
#            usflu <- get_flu_data("national", "ilinet", years=1997:2014)
        ...
        xreg <- San_Juan_test%>%
          select(c(air_temperature4,relative_humidity4,precipitation_amount4))
    ...
    saveRDS(seasonally_differenced_log_sarima_fit,
        file = file.path(
            "../../../inst/results",
            data_set,
            "estimation-results/sarima-fit_iqlag4.rds"))
}
...
save(sarima_predictions_df, file = "../../results/dengue_sj/prediction-results/sarima-predictions_iqlag4.Rdata")
...
ggplot() +
  ...
  geom_line(aes(x=317:468, y = prediction, colour = ph, group = 1), data = predictions_df[predictions_df$ph %in% 5, , drop = FALSE]) +
  geom_line(aes(x=1:312, y = dlag, group = 1, color ="fit")) +
  theme_bw() 

  
fit_data <- exp(as.numeric(seasonally_differenced_log_sarima_fit$x)) - 1
dlag <- lag(fit_data, n=50) 
