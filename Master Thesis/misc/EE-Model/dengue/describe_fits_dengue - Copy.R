...
fits_dengue$ar1 <- hhh4(I,
                        control = ctrls_dengue$ar1)

fits_dengue$ar2 <- profile_par_lag(I,
                                   control = ctrls_dengue$ar2)

fits_dengue$geom <- profile_par_lag(I,
                                    control = ctrls_dengue$geom)

fits_dengue$pois <- profile_par_lag(I,
                                    control = ctrls_dengue$pois)

fits_dengue$lin <- profile_par_lag(I,
                                   control = ctrls_dengue$lin)

fits_dengue$unres <- profile_par_lag(I,
                                     control = ctrls_dengue$unres)

fits_dengue$siraj <- hhh4_lag(I, control = ctrls_dengue$siraj)
