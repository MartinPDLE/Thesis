...
setwd("~/Documents/Uni/Epidemiology/Master Thesis/Thesis/Master Thesis/article-disease-pred-with-kcde/inst/code/estimation")
        ...
        Iquitos_test <- read.csv("../../../data-raw/Iquitos_total.csv")
...
predictions_df$week_start_date <- Iquitos_test$week_start_date[1]
...
for(predictions_df_row_ind in sarima_inds) {
	...
	last_obs_ind <- which(Iquitos_test$season == predictions_df$last_obs_season[predictions_df_row_ind] &
		San_Juan_test$season_week == predictions_df$last_obs_week[predictions_df_row_ind])
	
	predictions_df$week_start_date[predictions_df_row_ind] <- Iquitos_test$week_start_date[last_obs_ind + as.numeric(ph)]
  ...
  #updated_sj_log_sarima_fit <- Arima(new_data, model = sj_log_sarima_fit)
	...
	new_data <- log(Iquitos_test$total_cases[seq_len(last_obs_ind)] + 1)
...
#predictions_df$prediction[predictions_df_row_ind] <- exp(predictive_log_mean) - 1
	...
	predictions_df$AE[predictions_df_row_ind] <- abs(predictions_df$prediction[predictions_df_row_ind] - Iquitos_test$total_cases[last_obs_ind + ph])
	predictions_df$log_score[predictions_df_row_ind] <- dlnorm(Iquitos_test$total_cases[last_obs_ind + ph] + 1,
