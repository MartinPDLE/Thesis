...
for(lag_structure in names_lag_structures){
  ...
  for(ind in tps){
    ...
    file_name_fit <- paste0("model_fits/dengue_", lag_structure,
                            "/fit_dengue_", lag_structure, "_Iq", ind, ".rda")
    load(file_name_fit)

    fit_dengue_temp <- get(paste0("fit_dengue_", lag_structure, "_temp"))
    fit_dengue_temp$stsObj <- I # the stsObj of fit_dengue_temp contains NAs
    ...
    for(horizon in 1:max_horizon){
      if(horizon <= nrow(I@observed) - ind){
        capture.output(
          pred_temp <- forecasting_nStepAhead(fit_dengue_temp, stsObj = I, tp = ind,
  ...
  # write out results:
  write.csv(results_detailed_dengue[[lag_structure]], paste0("forecasts/forecasts_dengue_", lag_structure,"_Iq", ".csv"))
  # write out log scores:
  write.csv(logS_dengue[[lag_structure]], paste0("logS/logS_dengue_", lag_structure, "Iq", ".csv"))
}
