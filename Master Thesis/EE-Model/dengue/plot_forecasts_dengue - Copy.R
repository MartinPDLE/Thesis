...
forecasts_dengue_geom <- read.csv("forecasts/forecasts_dengue_geom_Iq.csv")
...
timepoints <- seq(from = I@start[1] +
                    (I@start[2] + tps[1] + max_horizon)/I@freq,
                  by = 1/I@freq, length.out = nrow(subs_dengue_geom[[1]]))
...
for(horizon in c(1, 2, 4, 8)){
  plot(NULL, xlim = range(timepoints), ylim = c(0, 150), xlab = "", ylab = "",
